    ...
    if (ret == 1) {
      switch (state) {
        ...
        case DECODE_PDSCH:
          ...
          // 需要预先加入4个RNTI的参数，分别是P-RNTI是FFFE, SI-RNTI是FFFF（FFF4）, M-RNTI是FFFD，其对于所有UE是共用的（其中FFF4在36.321-e21中也表示 SI-RNTI，
          // 下面函数解码当前指针中的控制信息
        	int nCount = srslte_ue_dl_get_control_cc(&ue_dl, &sf_buffer[prog_args.time_offset], data, srslte_ue_sync_get_sfidx(&ue_sync), 0, sfn);          
          //这里获取当前子帧中可能的rnti的值
          // 上个函数中已经对控制信息进行处理了，因此下面这个函数只需要处理C-RNTI获取数据信息即可？，但是上个函数没有具体解码消息的内容，
          //  可以根据需要多加入几个公共的RNTI参数。
          if(nCount >0)
          {
...
//        			printf("No decode\n");
            ...
            // 多条RAR消息的情况
              ...
              case 88:              
        			srslte_ue_dl_reset_rnti_user(&ue_dl, 256*data[(n/8)-5] + data[(n/8)-4]);
