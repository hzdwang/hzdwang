...
void srslte_ue_dl_reset_rnti_list(srslte_ue_dl_t *q) {
	...
	bzero(q->rnti_use_count, sizeof(q->rnti_use_count));
	for (int i = 1; i <= 10; i++) {
		q->rnti_list[i] = 1;
	}
	q->rnti_list[65534] = 1;
	q->rnti_list[65535] = 1;
  q->listSize = 12;// 10个RARNTI+FFFF（BROAD）和FFFE（PAGING）
  
  q->rnti_activeCount= 0;
  bzero(q->rnti_active,sizeof(q->rnti_active));
  bzero(q->rnti_active_temp,sizeof(q->rnti_active_temp));
  q->rnti_activetempCount=0;
  q->cRNTICount = 0;
}
//判断rnti是否在活跃列表中，如果不在，加入列表,这里同上面的处理方式不同，记录具体的值
// 此时需要更新列表，更新方式为最少使用（成功解码数据最少的去除）
void srslte_ue_dl_set_active_crnti(srslte_ue_dl_t *q, uint16_t user)
{
  q->cRNTICount ++;
  if(q->rnti_activeCount < MAXACTIVECRNTI)
  {
      q->rnti_active[q->rnti_activeCount++] = user;
  }
  else
  {
    // 处理特定的更新机制
...
void srslte_ue_dl_reset_rnti_user(srslte_ue_dl_t *q, uint16_t user) {
	...
	q->rnti_use_count[user]++;
  if(!rnti_in_list(q,user))q->listSize ++;
}

void srslte_ue_dl_reset_rnti_userPWH(srslte_ue_dl_t *q, uint16_t user) {
  if(q->listSize <MAXACTIVERNTI)
  {
    // 如果不在列表中，需要填入，否则只需要计数
    if(!rnti_in_list(q,user))
    {
      q->rnti_list[user] = 2;
      q->rnti_use_count[user]++;
      q->rnti_active_temp[q->rnti_activetempCount++] = user;
      q->listSize ++;
    }
    else
    {
...
void srslte_ue_dl_reset_rnti_userPWH_clear(srslte_ue_dl_t *q) {
  // 清理所有临时的RNTI
...
void srslte_ue_dl_reset_rnti_userRAR(srslte_ue_dl_t *q, uint16_t user) {
  if(q->listSize <MAXACTIVERNTI)
  {    
    if(!rnti_in_list(q,user))
    {
      q->rnti_list[user] = 3;// 2 类型的需要更改
      q->rnti_use_count[user]++;// 被使用的次数吗
      q->listSize ++;
    }
    else {      
      q->rnti_list[user] = 3;// 2 类型的需要更改
...
void srslte_ue_dl_reset_rnti_userPWHToRAR(srslte_ue_dl_t *q, uint16_t user) {
  q->rnti_list[user] = 3;// 2 类型的需要更改
  //q->rnti_use_count[user]++;// 被使用的次数吗？  
  for (size_t i = 0; i < q->rnti_activetempCount; i++)
  {
    // 查找
    if(user == q->rnti_active_temp[i])
    {
      // 移除
      for (size_t j = i; j < q->rnti_activetempCount-1; j++)
      {      
        q->rnti_active_temp[j] = q->rnti_active_temp[j+1];
      }
      q->rnti_activetempCount--;
      break;      
    }
  }  
  //q->rnti_active[q->rnti_activeCount ++] = user;
  srslte_ue_dl_set_active_crnti(q,user);
}

void srslte_ue_dl_reset_rnti_user_to(srslte_ue_dl_t *q, uint16_t user, uint16_t val) {
	q->rnti_list[user] = val;
}
// 用于定时更新活跃的列表,不处理长期存在的，经常更新，防止影响现有的处理效率
void srslte_ue_dl_update_rnti_list(srslte_ue_dl_t *q) {
	
  // 先备份活跃列表  
  // uint16_t rnti_active[64];
  // uint32_t rnti_activeCount;
  // for (size_t i = 0; i < q->rnti_activeCount; i++)
  // {
  //   rnti_active[i] = q->rnti_active[i];
  // }
  q->rnti_activeCount= 0;
  bzero(q->rnti_active,sizeof(q->rnti_active));
  ...
  for (int i = 10; i < 65533; i++) {
    ...
    else if(q->rnti_list[i]==3)
    {
      if(q->rnti_use_count[i]<3)
      {
        q->rnti_list[i] =0;
        q->rnti_use_count[i] =0;
        q->listSize --;
        ...
        q->rnti_active[q->rnti_activeCount++] = i;
      }
      //需要判断是否被使用过，如果使用过，保留；如果没有使用过，清除，这里暂时不处理，假设长期存在
    }
	}
}
...
int srslte_ue_dl_decode2(srslte_ue_dl_t *q, cf_t *input, uint8_t *data, uint32_t sf_idx,uint32_t sfn) {

  for (size_t i = 0; i < 65536; i++)
  {
       if(rnti_in_list(q,i)>0)
      {
        int n = srslte_ue_dl_decode_rnti_rv(q, input, data, sf_idx, i, 0);
        if(n>0)
        {
            FILE *fw = fopen("tempDl.txt","a");
            //记录系统帧号，子帧编号，RNTI值和相关的数据
            fprintf(fw,"SFN=%4d,subFrame=%2d,RNTI=%04x",sfn,sf_idx,i);
                                ...
                                uint32_t cfi, uint32_t sf_idx, uint16_t rnti, uint32_t rvidx) 
{
  ...
  if (q->pdsch_cfg.grant.mcs.mod >= 0 && q->pdsch_cfg.grant.mcs.tbs >= 0) {
    ...
    if (ret == SRSLTE_ERROR) {
      ...
      // FILE *fw = fopen("tempDl.txt","a");
      // srslte_vec_fprint_byte(fw,data,q->pdsch_cfg.grant.mcs.tbs/8);
      // //srslte_vec_fprint_hex(fw, data, q->pdsch_cfg.grant.mcs.tbs);
      // fclose(fw);
    }
    q->pkts_total++;
  }
...
static int dci_blind_search(srslte_ue_dl_t *q, dci_blind_search_t *search_space, uint16_t rnti, srslte_dci_msg_t *dci_msg) 
{
  ...
  if (rnti) {
    ...
    while (!ret && i < search_space->nof_locations) {
      ...
      // printf("RNTI(%d),CRC_rem(%d) Msg:\n",rnti,crc_rem);
      if(crc_rem >0)
      {
      // for (size_t i = 0; i < dci_msg->nof_bits; i++)
      // {
      //   printf("%d",dci_msg->data[i]);
      //   /* code */
      // }      
      // printf("\n");
      }
  ...
  for (int i=0;i<nof_locations;i++) {
	  ...
	  for (int f=0;f<nof_formats;f++) {
		  if (!locations[i].checked) {
            ...
            if(lprob[i] >=PROB_THR){
					  //srslte_ue_dl_reset_rnti_user(q, crc_rem);// 通过能量的方法不一定准确
            srslte_ue_dl_reset_rnti_userPWH(q,crc_rem);
					  ...
					  if ((crc_rem >= 1 && crc_rem <= 0xffff)) { // if ((crc_rem >= 0 && crc_rem <= 0x0010)) { //
						...
						ret++;            
            // 下面一句更新当前的rnti
						if (crc_rem > 0x0000 && crc_rem <= 0x000a && formats[f]!=SRSLTE_DCI_FORMAT0) 
            {
              q->current_rnti = crc_rem;
              printf("New RA-Rnti :%04x\n",crc_rem);
            }
						
            // 此时获取到正确的rnti值            
            //是否应该在此加入解码函数？
            //int  n = srslte_ue_dl_decode(q, &sf_buffer[prog_args.time_offset], data, srslte_ue_sync_get_sfidx(&ue_sync));
            //解码函数结束
