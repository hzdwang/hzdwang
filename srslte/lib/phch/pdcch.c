...
static float dci_decode_and_check_list(srslte_pdcch_t *q, float *e, uint8_t *data, uint32_t E, uint32_t nof_bits, uint16_t *crc, uint8_t *list) {
	  ...
	  //(nof_bits +16)*3 <= E) // check if it is possbile to store the dci msg in the available cces
  {
    ...
    // printf("OrgData:\n");
    // for (size_t i = 0; i < nof_bits+16; i++)
    // {
    //   printf("%d",data[i]);
    // }
    // printf("\n");    
...
//    printf("op %d crnti %d\n",p_bits ^ crc_res,c_rnti);
    // printf("p_bits: 0x%x, crc_checksum: 0x%x, crc_rem: 0x%x\n", p_bits, crc_res,
    //     c_rnti);
    ...
    // else{
    //   //从列表中移除该值 ?，不可以，因为这里只是尝试用其取解码，这个子帧中没有传数据，不表示这个RNTI不再使用。
    //   if((*crc >10) &&(*crc <65534))
    //         list[*crc]=0;
    // }
    }
                            ...
                            uint16_t *crc_rem) 
{
      ...
      srslte_dci_location_isvalid(location))
  {
    if (location->ncce * 72 + PDCCH_FORMAT_NOF_BITS(location->L) > 
      ...
      //  printf("Decoded DCI: nCCE=%d, L=%d, format=%s, msg_len=%d, mean=%f, crc_rem=0x%x\n",
      //       location->ncce, location->L, srslte_dci_format_string(format), nof_bits, mean, *crc_rem);
